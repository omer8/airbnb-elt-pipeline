...
* **Amazon S3**: Stores raw Airbnb CSV files (listings, hosts, bookings) ingested into Snowflake via `COPY INTO`.
* **Docker & Docker Compose**: Containerizes the full Airflow stack for local development.
* **PostgreSQL**: Airflow metadata database.
* **Redis**: Celery message broker for distributed task execution.
* **SMTP / Gmail**: Sends email notifications on pipeline success or failure, including the exact failed task name and log URL.
...
2. `S3KeySensor` detects new files and triggers ingestion tasks in parallel.
3. Snowflake `COPY INTO` loads raw data into staging tables with explicit column casting.
...
3. Email alerts are sent on success or failure with full run details and log URL.
...
* `dags/`: Main Airflow DAG — `airbnb_elt_pipeline.py`.
...
* `dbt/airbnb-analytics/models/gold/`: Star schema — dimensions and fact tables.
